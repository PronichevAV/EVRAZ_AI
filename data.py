...
def preprocessing(encode_cat = False):
    ...
    #num_columns += ['VES25КШ', 'VESК   ', 'VESКП  ', 'VESКШС8', 'VESЛЧ  ', 'VESО   ', 'VESОК  ', 'VESУ2КШ']
    ...
    o2_timediff_pivot_train = df_chronom_train[df_chronom_train['NOP'] == 'Продувка'][['NPLV','VR_NACH', 'VR_KON', 'time_diff', 'time_diff_sec']].set_index('NPLV')
    ...
    o2_timediff_pivot_test = df_chronom_test[df_chronom_test['NOP'] == 'Продувка'][['NPLV','VR_NACH', 'VR_KON', 'time_diff', 'time_diff_sec']].set_index('NPLV')
    ...
    num_columns += ['produv_mean_RAS', 'produv_mean_POL', 'produv_sum_RAS', 'produv_sum_POL', 'produv_min_RAS', 'produv_min_POL',
    ...
    o2_final_pivot_train = pd.merge(o2_pivot_train ,o2_timediff_pivot_train, left_index=True, right_index=True).reset_index()
    o2_final_pivot_test = pd.merge(o2_pivot_test ,o2_timediff_pivot_test, left_index=True, right_index=True).reset_index()
    
    o2_final_pivot_test.columns = ['NPLV', 'chronom_O2', 'chronom_produv_NACH', 'chronom_produv_KON', 'chronom_produv_td', 'chronom_produv_timediff']
    o2_final_pivot_train.columns = ['NPLV', 'chronom_O2', 'chronom_produv_NACH', 'chronom_produv_KON', 'chronom_produv_td', 'chronom_produv_timediff']
    ...
    train = pd.merge(train, time_between_plavki(df_chronom=df_chronom_train), how='left', left_on='NPLV', right_on='NPLV')
    ...
    # Дроп лишних колонок
    print(train.columns)
    bad_columns = ['DATA_ZAMERA', 'plavka_VR_NACH', 'plavka_VR_KON','index_x', 'index_y', 'VES8', 'index', 'chronom_produv_NACH', 'chronom_produv_KON', 'chronom_produv_td']
...
def sip_weights_df(df):
    ...
    df.reset_index().set_index('index')
    # Drop rare columns
    df.drop(['VSSYP344', 'VSSYP11'], errors='ignore', axis = 1, inplace = True) # Обоих нету в трейне, 11 очень мало в принципе
    # All nans are zero
    df.fillna(0, inplace = True)
    return df
